...
#include <string>//std::string replaces QString for all data passed to Model and FileManager constructors
...
void SignupForm::setupUI()
{
    ...
    //add emojis to labels and buttons for a more engaging and friendly UI, while keeping the text clear and concise.
    ...
    //the status label is used to display validation messages or success messages after the user attempts to register. It is centered and styled to stand out, making it easy for users to see feedback about their registration attempt. For example, if the user leaves required fields empty, enters mismatched passwords, or chooses a username that already exists, the status label will show an appropriate error message in red. If the registration is successful, it will show a success message in green.
    ...
    //the form layout organizes the input fields and their labels in a clean, structured manner. Each row consists of a label (e.g., "Full Name:") and its corresponding input field. The spacing between rows is set to 10 pixels to ensure that the form doesn't look too cramped, while still keeping related fields visually grouped together.
    ...
    //the main layout of the form is a vertical box layout that organizes the title, form, status label, and buttons in a vertical manner. The contents margins are set to 40 pixels on the left and right, and 30 pixels on the top and bottom, to create a visually balanced and spacious layout.
...
{
    //User::passwordStrength now takes std::string; convert at the boundary
    int strength = User::passwordStrength(text.toStdString());
    barStrength->setValue(strength);

    //QStringList hints replaced with a plain fixed-size array of const char*
    //to avoid using Qt list types, as per project requirements
    const char* hints[] = {
        "Very Weak — too short",
        "Weak — add uppercase",
        "Fair — add a number",
    ...
    //index is guaranteed 0–4 because barStrength range is 0–4
...
void SignupForm::onRegisterClicked()
{
    ...
    //read Qt widget values and convert to std::string at the boundary
    ...
    std::string confirm = txtConfirmPassword->text().toStdString();
    ...
    std::string role = cmbRole->currentText().toStdString();
    int         age = txtAge->text().toInt();

    //validation — all comparisons now use std::string
    ...
    //FileManager::userExists now takes std::string
    if (FileManager::userExists(username)) {
        lblStatus->setText("❌ Username already taken. Choose another.");
        lblStatus->setStyleSheet("color: #e74c3c;"); return;
    }

    //User constructor now takes std::string parameters
    User newUser(username, password, role);
    FileManager::saveUser(newUser);

    //create Donor or Patient based on selected role; all constructors take std::string
    if (role == "Donor") {
